...
const batteries = (displaySnapshot.batteries || []).map((battery) => {
    ...
    return {
        ...
        soc: percent(battery.socCapacityIntegerPercent),
        socReported: percent(battery.socReportedPercent ?? battery.soc),
        socCapacity: bounded(battery.socCapacityRawPercent ?? battery.capacitySoc, 0, 105, 2),
        socCapacityInteger: percent(battery.socCapacityIntegerPercent),
        socInterpolated: bounded(battery.socInterpolatedPercent, 0, 100, 2),
        socInterpolationState: battery.socInterpolationState || '—',
        socInterpolationAnchor: battery.socInterpolationAnchor || '—',
        socChargeGain: number(battery.socChargeGain, 3),
        socDischargeGain: number(battery.socDischargeGain, 3),
        socFullReset: battery.socFullReset === true,
        socEvents: battery.socEvents || [],
        ...
        remainingCapacityRawMah: count(battery.remainingCapacityRawMah),
        totalCapacityRawMah: count(battery.totalCapacityRawMah),
        delta: number(battery.systemVoltageDeltaMv ?? battery.system_voltage_delta_mv, 1),
        vmin: bounded(battery.vmin, 2, 4.5),
        vminLocation: cellLocation(battery.vminId, bounded(battery.vmin, 2, 4.5)),
        vmax: bounded(battery.vmax, 2, 4.5),
        vmaxLocation: cellLocation(battery.vmaxId, bounded(battery.vmax, 2, 4.5)),
        cellSpread: number(battery.spread),
...
msg.payload = {
    ...
    aggregate: {
        ...
        socIntegerMinimum: percent(displaySnapshot.capacitySoc?.integerMinimum),
        socIntegerMaximum: percent(displaySnapshot.capacitySoc?.integerMaximum),
        socIntegerSpread: number(displaySnapshot.capacitySoc?.integerSpread, 0),
        socInterpolatedMinimum: number(displaySnapshot.capacitySoc?.interpolatedMinimum, 2),
        socInterpolatedMaximum: number(displaySnapshot.capacitySoc?.interpolatedMaximum, 2),
        socInterpolatedSpread: number(displaySnapshot.capacitySoc?.interpolatedSpread, 2),
        socLeaders: (displaySnapshot.capacitySoc?.leaders || []).join(', ') || '—',
        socTrailers: (displaySnapshot.capacitySoc?.trailers || []).join(', ') || '—',
        socEvents: displaySnapshot.capacitySoc?.events || []
    },
    inventory: {
